        ...
        for picNO in range(0,10):
            with torch.no_grad():
                ...
                def addPaddingPhase(phase, pad=padding, size=N):
                    ...
                    inIn = getInsideIndex(pad, size)
                    temp[inIn:inIn+size, inIn:inIn+size] = phase
                    return temp
                ...
                # holo = IFT((FT(holo_gt_b**2)) * np.conj(prop))
                # fucker1 = abs(holo)
                # fucker2 = np.angle(holo)
                # shit = fucker1 * np.exp(1j * fucker2)
                # dogshit = abs(IFT((FT(shit)) * prop))
                #
                # img_rec_pow = np.squeeze(abs(dogshit) ** 2)
                ...
                # psnr_amp, ssim_amp, mse_amp = countPSM(amp_r, amp_gt_b)
                # psnr_phase, ssim_phase, mse_phase = countPSM(phase_r_small, phase_gt_small)
                #
                # step=996
                # print(
                #     f"picNo {picNO} holo {step}  psnr {'%.4f' % psnr_holo}  ssim {'%.4f' % ssim_holo}  mse {'%.4f' % mse_holo}")
                # print(
                #     f"amp {step}                                                   psnr {'%.4f' % psnr_amp}  ssim {'%.4f' % ssim_amp}  mse {'%.4f' % mse_amp} ")
                # print(
                #     f"phase {step}                                                                                          psnr {'%.4f' % psnr_phase} ssim {'%.4f' % ssim_phase}  mse {'%.4f' % mse_phase}")

                # region else
                savePath = 'result_1k_m4/test'
                gap = 'auto'
                keep_size = 400
                startStep=1600
                endStep = 2100
                ...
                # holo = holo_gt_b[inIn:inIn+N,inIn:inIn+N]
                # holo_gt = np.abs(holo)

                holo_sparse_b = copy.deepcopy(holo_gt_b)
                # holo_sparse_b[inIn:inIn+N, inIn:inIn+N] = addNoise(holo_gt, size=keep_size)
                holo_sparse_b = addNoise(holo_gt_b, size=keep_size, picSize=padding)
                ...
                for i in range(startStep,endStep):
                    ...
                    holo_sparse_b = DC(holo_sparse_b, holo_gt_b, size=keep_size,picSize=padding)
                    ...
                    # **************************** NET START ***********************
                    amp_phase = np.concatenate([object_b[None,inIn:inIn+N,inIn:inIn+N], phase_b2[None,inIn:inIn+N,inIn:inIn+N]], axis=0)
                    ...
                    if i % 1 == 0:
                        ...
                        if ifSavePng and i % 10 == 0:
                            ...
                            savemat(f'./{savePath}/{title}_phase_size_{keep_size}_gap_{gap}_iter/{picNO}/holo/{step}.mat', mdict={'data': phase_r})
